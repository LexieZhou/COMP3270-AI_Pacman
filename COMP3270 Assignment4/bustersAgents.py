    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ghostDistances = {}   # store living ghost position and their distance to the pacman
        for ghost in livingGhostPositionDistributions:
            ghostPos = ghost.argMax() # find the most likely ghost position
            ghostDis = self.distancer.getDistance(pacmanPosition, ghostPos)
            ghostDistances[ghostPos] = ghostDis
        # find the closest ghost
        closestGhostPos = min(ghostDistances, key=ghostDistances.get)
        # choose an action to obtain closest mazeDistance
        actionTodistance = {}   # store action and relative distance
        ...
        for a in legal:
            ...
            actionTodistance[a] = distance
        return min(actionTodistance, key=actionTodistance.get)
